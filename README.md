...
- 单一页面滚动：合并结果和运行日志都随主页面自然展开，不使用面板内部滚动框。
...
3. 点击“开始合并”。应用会将所有有效 PDF 按高度排序并动态尝试最高与最低配对；若缩放匹配失败，最高发票单独生成 A4 PDF，最低发票返回队列继续尝试。若数量为奇数，初始最高发票直接单独成页。输出目录中的同名文件会在生成成功后被替换。
4. 在“合并结果”区域确认每组源 PDF 和输出文件。
5. 选择目标打印机，点击“打印处理结果”，提交本次成功合并的文件以及需要单独打印的最高 PDF。
